...
Everything this application may need to replace sits behind one of three seams:
...
- Never read or write `.env*` (the `.example`, `.sample` and `.template` variants are
  ...
  checkout can hold a real provider credential, so reading one is already a disclosure
  whether or not anything is written back: no `cat`, no `grep`, no copy to a temp path,
  and never a value out of it onto a command line. `src/server/env.ts` is the list of
  names the process reads, and `.env.example` ships every one of them with an empty
  value — those two are what to open when you need to know what exists.
