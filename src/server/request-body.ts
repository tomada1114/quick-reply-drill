 ...
 * or middleware — and a self-hosted `next start` enforces no payload limit of
 * its own, which is the deployment this has to be safe in. 64 KiB is small
 * enough that concurrent requests cannot exhaust memory between them, and far
 * above the largest body any endpoint here accepts: every field each request
 * schema declares is bounded in `src/core/wire.ts`, and the longest legal body
 * among them is a few kilobytes of JSON. The headroom is deliberate — a
 * ceiling under what a schema calls legal would make a legal request
 * unreachable — and this number is the one a client is promised, so lowering
 * it is a contract change rather than a tuning detail.
 */
